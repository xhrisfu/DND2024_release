import{mv as b}from"./eventTargetElement-Dx7FE-gh.js";const v=[{tag:"intent",label:"玩家意图解析",prompt:`只从最后一条【玩家本轮输入】识别本轮任务，不能把历史对话中的动作当作新指令。
若本轮输入是【战斗结束·胜利/败退】，它是引擎已结算的战果：承接战后状态，不重演开战，不重新生成敌人，也不补写战斗动作或掷骰结果。
本轮输入说的是：
...
判断玩家的口吻是否是命令式口吻，命令式口吻（"守卫放我进去了"）一律降格为"尝试"：`,sensitivity:"visible",resident:!0},{tag:"stop_check",label:"剧情分析",prompt:`概括最近20轮的剧情发生了什么（依据【近期剧情梗概】与上文对话历史，没有的别编）：
                 玩家角色（PC）的行为是否需要检定：
                 需要检定→检定前的正文要演到哪个悬念点停笔（说辞刚出口/手刚伸向锁孔/剑刚出鞘——只演出手，不写结果）：`,sensitivity:"visible",resident:!0},{tag:"canon_beat",label:"模组承接",prompt:`依据当前实际区域、可用出口和已经落账的事件回应玩家，不把书籍顺序当作游玩顺序。
                 当前内容承担危险、交流、调查、后果或收束中的什么作用？战斗结束后先承接环境变化、幸存者反应和交流机会，再交回选择。
                 条件不成立的事件不演出；遇敌、敌对态度和候选名单不等于交战。帮助、交涉与已经接受的陪同在离开叙事片段后仍有效。
                 玩家实际移动到相邻区域用 move_party 的 toLocationId；返回切片列出的已知地点也可直接填写最终 toLocationId，由引擎沿开放旧路连续推进，无需要求玩家逐个确认中间地点。其它长途分段旅行用 travel_to。同一实际区域的叙事焦点变化可以用 toSceneId，但不是旅行，也不代表开战。
                 只引用切片提供的合法 id、可用通路和已满足条件的内容；不要自编替代内容，不要求清空所有遭遇。确认移动成功后才描述抵达，目的地交互等待新切片。
                 任务逐目标记录；已配置条件的目标、章节成果与里程碑由引擎自动结算。开放语义目标只申报实际完成的 objectiveId，不能用一次 advance_quest 代替整项任务。物品真正交付时用 remove_item 并填写 toNpcId，成功出库才记交付。
                 亲密/nsfw场景进行中不引入无关模组事件打断，待自然收束后再处理。`,sensitivity:"folded",resident:!0,materials:["sceneSlice"]},{tag:"persona_anchor",label:"角色一致",prompt:`照【主角设定】一句话复述主角是谁（身份基线行；块没注入就写"无"）：
                 ...
                 有没有与设定冲突的写法（把长寿种族写老、让角色说不符合身份教养的话、性格无故漂移、遗忘既有关系或背景钩子；涉及祈祷/誓言/神祇时是否写错主角信仰、或让秘密信仰被无端暴露），有则先改再落笔：`,sensitivity:"folded",resident:!0,materials:["characterBaseline"]},{tag:"tool_plan",label:"工具规划",prompt:"只列出本段确定要实际调用的工具及理由，不复述可用工具目录或未采用的候选工具；无需调用时只写“无”：",sensitivity:"visible",resident:!0,commitment:!0},{tag:"consequence",label:"剧情规划",prompt:`战役下一个里程碑节点是：
                 目前的中短期剧情走向是：
                ...
                `,sensitivity:"folded",resident:!0},{tag:"scene_gear",label:"场景档位",prompt:`本段剧情属于什么类型：（日常／探索／社交／nsfw／大战／余波）。
                 ...
                 对有阵营设定的角色，同时读取守序/中立/混乱与善良/中立/邪恶两轴，落到本轮目标和手段；相同提议在不同阵营下可能得到不同反应。职业和好感不得把全队演成善良；偏离常态要有已有事实支撑，不编造底线：
                 其中谁会在这一轮主动做点什么（推进自己的事、提出主张、插话、反对、离开、表态），具体是什么——主动不等于抢戏，一两人有动作即可，无事可做的不硬安排；绝不替玩家决定或代玩家行动：
                 谁已久未开口（看「在场发言」，没有这一行就写"无账可查"）：若久未发声者此刻有立场或情绪可表达，让 TA 这轮自然冒泡；无话可说的不硬塞台词。`,sensitivity:"folded",resident:!1,trigger:{signal:"namedNpcPresent",slot:"scene"},materials:["panel"]},{tag:"companion_bond",label:"队友好感",prompt:`这一段玩家的言行，在队里谁会真的往心里去（对照盘面队友档的九宫格阵营两轴、看重/厌恶、既定关系与当前档位）：
                 先按本人目标与可接受手段判断赞成/反对，再定好感方向；邪恶者可因有利恶行增加好感，混乱者可因强制束缚降低好感，守序者可因守信增加好感；中立者按具体利益与牵挂判断，不统一套善良尺度，也不为每轮硬凑变化：
...
（对白独立成行写成 名字：「台词」，心声写 名字（心声）：「…」；不要把台词嵌进散文段。）`,sensitivity:"visible",resident:!0}],d=3;function y(t,i){return(t.materials??[]).every(n=>{var r;return((r=i.availableMaterials)==null?void 0:r[n])===!0})}function h(t,i){var r,e;const s=((r=t.trigger)==null?void 0:r.cooldown)??0;if(s<=0)return!0;const n=(e=i.utterancesSinceLast)==null?void 0:e[t.tag];return n===void 0||n>=s}function T(t={}){const s=(t.registry??v).filter(e=>{var l;return e.pending||!y(e,t)?!1:e.resident?!0:!e.trigger||((l=t.signals)==null?void 0:l[e.trigger.signal])!==!0?!1:h(e,t)}),n=s.filter(e=>{var l;return!e.resident&&((l=e.trigger)==null?void 0:l.slot)==="counted"});if(n.length<=d)return s;const r=new Set([...n].sort((e,l)=>{var a,c;return(((a=e.trigger)==null?void 0:a.priority)??Number.MAX_SAFE_INTEGER)-(((c=l.trigger)==null?void 0:c.priority)??Number.MAX_SAFE_INTEGER)}).slice(0,d).map(e=>e.tag));return s.filter(e=>{var l;return e.resident||((l=e.trigger)==null?void 0:l.slot)!=="counted"||r.has(e.tag)})}function N(t){return t.map(i=>`<${i.tag}>${i.prompt}</${i.tag}>`).join("")}const _=[{tag:"digest",label:"回执消化",prompt:"逐条写下刚才每个工具回执的结果（成败／关键数字），以回执为准不凭印象：",sensitivity:"visible"},{tag:"check_outcome",label:"检定承接",prompt:`本次检定的结果是：
...
还有点名了但未调用、或需要补调的工具吗（没有写"无"，有就现在调用）：`,sensitivity:"folded"}];function I(t){return _.filter(i=>i.when===void 0?!0:i.when==="check_success"?t.checkOutcome==="success":t.checkOutcome==="failure").map(i=>({tag:i.tag,...i.label!==void 0?{label:i.label}:{},prompt:i.prompt,sensitivity:i.sensitivity,resident:!0}))}function R(t,i,s,n){const r=i.filter(a=>a.commitment).map(a=>a.tag);if(r.length===0)return[];const e=[];for(const a of r){const c=new RegExp(`<${a}>([\\s\\S]*?)</${a}>`,"g");let o;for(;(o=c.exec(t))!==null;)e.push(o[1])}if(e.length===0)return[];const l=e.flatMap(a=>a.split(`
`));return s.filter(a=>!n.has(a)&&l.some(c=>{const o=[...c.matchAll(new RegExp(`\\b${a}\\b`,"g"))];return o.length===0||s.filter(g=>new RegExp(`\\b${g}\\b`).test(c)).length===1&&/(?:无需|不需要|不再)(?:重复)?调用[。.!！\s]*$/.test(c)?!1:o.some(g=>{const p=c.slice(0,g.index).replace(/[`\s]/g,"");return!/(?:无需|不需要|不再|不要|不)(?:重复)?(?:调用)?(?:default_api:)?$/.test(p)&&!/(?:撤销|取消)(?:调用)?(?:default_api:)?$/.test(p)})}))}const u={first:300,resume:150},w=50,E=500;function C(t,i){const s=Number.isFinite(t)&&t>0?t:u.first,n=i.filter(r=>!r.resident).length;return Math.min(s+w*n,E)}function k(t,i,s,n,r){const e=s.filter(o=>o.trim().length>0),l=i.map(o=>{const m=new RegExp(`<${o.tag}>([\\s\\S]*?)</${o.tag}>`).exec(t);if(!m)return{tag:o.tag,sensitivity:o.sensitivity,missing:!0,text:"",hollow:!1};const g=m[1].trim(),p=e.length>0&&!e.some(f=>g.includes(f));return{tag:o.tag,sensitivity:o.sensitivity,missing:!1,text:g,hollow:p}}),a=b(t),c=r!==void 0&&Number.isFinite(r)&&r>0?r:u[n];return{steps:l,missingCount:l.filter(o=>o.missing).length,hollowCount:l.filter(o=>o.hollow).length,tokenEstimate:a,overTokenLimit:a>c}}function x(t,i){if(!t)return[];const s=new Map(i.map(n=>[n.tag,n.label??n.tag]));return[...new Set(t.steps.filter(n=>n.missing).map(n=>s.get(n.tag)??n.tag))]}function P(t,i){const s=[];let n=t;for(const e of i){const l=new RegExp(`<${e.tag}>([\\s\\S]*?)</${e.tag}>`),a=l.exec(n);a&&(s.push({tag:e.tag,sensitivity:e.sensitivity,text:a[1].trim()}),n=n.replace(l,""))}const r=n.replace(/<[^>]+>/g,"").trim();return r.length>0&&s.push({tag:null,sensitivity:"visible",text:r}),s}export{v as C,_ as R,I as a,N as b,C as c,k as d,R as f,x as m,T as p,P as t};
